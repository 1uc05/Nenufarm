...
#define DC_VALUE_TO_TEST_ADC                            10      /* Valeur du DC pour tester l'ADC au démarrage aprés une CriticalError */
...
void AquaLight::v_AquaLight_configuration(const SETTINGS st_setttings)
{
     ...
     * - Si premier lancement du soft, les limites sont calculées aprés v_AquaLight_defineADCValue(), appelé dans le Handler
...
void AquaLight::v_AquaLight_event(const SCHEDULED_EVENT en_scheduledEvent)
{
    ...
    switch(m_en_currentStatus) {
        ...
        /* Mode pour tester l'ADC aprés un démarrage avec une CriticalError */
        ...
        default:
            switch(en_scheduledEvent) {
                ...
                case STOP_AQUALIGHT_CYCLE:
                    ...
                    } else {/* Nothing to do - Les plantes sont déjé au max */}
                    break;
...
{    
    ...
    if(m_ui16_readingHWTimer >= READING_HW_DELAY) {
        ...
        if(b_drvHardware_getPinValue(GPIO_PORTA, 4)) {
            ...
            /* Re vérification de l'alimentation 12.6V de la carte, si NOK aprés les mesures (PGOOD_12VDC = 0), l'erreur est écrasée par l'ancien état */
            if(!b_drvHardware_getPinValue(GPIO_PORTA, 4)) {
...
{
    /* Définition du palier min et max lors la lumière est allumèe */
    switch(en_intensityModeName) {
        case LUMEN_20:
